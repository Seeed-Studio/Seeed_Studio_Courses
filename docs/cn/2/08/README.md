...
> 注意：OpenCV 默认使用 BGR 颜色顺序而不是通常的 RGB 顺序。这是一个历史遗留问题，在使用时要特别注意这一点。
>
...
def display_color_spaces(image_path):
    ...
    hsv_rgb = cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB)  # 将 HSV 转换为 RGB 以便于正确显示，因为人眼更适应 RGB 颜色空间。
...
运行结果会显示一个包含三个子图的窗口，分别展示原始 RGB 图像、灰度图像和 HSV 图像的效果。
...
```plain
我需要开发一个图像分析工具，具有以下功能：
1. 读取并显示图像
2. 分析图像的基本信息（尺寸、通道数等）
3. 生成颜色分布直方图
4. 将分析结果可视化并保存
5. 所有结果需要整合到一个统一的显示界面中

请使用 OpenCV、Matplotlib 和 NumPy 实现这个工具。
```
...
```plain
请帮我实现一个图像预处理器，需要具备以下功能：
1. 支持从文件读取图像
2. 将图像调整为指定的目标尺寸（例如 224×224）
3. 对图像进行基本的增强处理：
   - 亮度调整
   - 对比度调整
4. 使用 Matplotlib 实时显示原始图像和处理后的效果对比
5. 支持将处理后的图像保存到指定目录
```
